...
class Bacteria(object):
    ...
    def ServerBlockEntityTickEvent(self, data, playerId):
        ...
        if blockEntityData and blockEntityData["root"]:
            ...
            if root in self.bacteriaDict and self.bacteriaDict[root] > 0:
                flag = 0
                flag = flag + self.createNew(comp, (posX, posY - 1, posZ), target, root)
                flag = flag + self.createNew(comp, (posX, posY + 1, posZ), target, root)
                flag = flag + self.createNew(comp, (posX - 1, posY, posZ), target, root)
                flag = flag + self.createNew(comp, (posX + 1, posY, posZ), target, root)
                flag = flag + self.createNew(comp, (posX, posY, posZ + 1), target, root)
                flag = flag + self.createNew(comp, (posX, posY, posZ - 1), target, root)
                if flag > 0:
    ...
    def createNew(self, comp, pos, target, root):
